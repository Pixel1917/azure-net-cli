...
const LAYERS: LayerName[] = ['domain', 'application', 'infrastructure', 'presentation'];
const FORBIDDEN_LAYER_IMPORTS: Record<LayerName, LayerName[]> = {
...
const getLayerFromPath = (filePath: string, contextRoot: string): LayerName | null => {
...
const getLayerFromAliasImport = (importPath: string, context: ContextConfig): LayerName | null => {
...
const getLayerFromRelativeImport = (filePath: string, importPath: string, contextRoot: string): LayerName | null => {
...
const createMessage = (sourceLayer: LayerName, targetLayer: LayerName, context: ContextConfig): string =>
	`Layer import violation: "${context.name}" ${sourceLayer} layer cannot import from ${targetLayer} layer`;

export default async function checkLayerImports(): Promise<void> {
