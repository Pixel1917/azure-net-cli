...
const isInside = (targetPath, rootPath) => targetPath === rootPath || targetPath.startsWith(`${rootPath}${path.sep}`);
const resolveAbsoluteRelativeImport = (filePath, importPath) => {
	if (!importPath.startsWith('.')) return null;
...
export default async function checkLayerBoundaries() {
	...
	for (const context of contexts) {
		...
		const allowedAliases = new Set([context.alias]);
