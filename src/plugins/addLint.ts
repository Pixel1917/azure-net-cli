...
interface ConfigState {
	...
	content: string | null;
	hasPackageManagerKey: boolean;
	validPackageManager: PackageManager | null;
}
...
const resolveRunScriptCommand = (manager: PackageManager, script: string): string => {
	switch (manager) {
		case 'pnpm':
			return `pnpm ${script}`;
		case 'npm':
...
const resolveHuskyCommand = (manager: PackageManager): string => {
	switch (manager) {
		case 'pnpm':
...
const detectPackageManager = (cwd: string): PackageManager | undefined => {
...
const parseConfigState = (cwd: string): ConfigState => {
	...
	const validPackageManager = managerMatch?.[1] as PackageManager | undefined;

	return {
		configPath,
		content,
		hasPackageManagerKey,
		validPackageManager: validPackageManager ?? null
	};
};

const upsertPackageManagerInConfig = (content: string, manager: PackageManager): string => {
	...
	return content.replace(/export\s+default\s*\{([\s\S]*?)\}\s*;?/, (_full, body: string) => {
...
const selectPackageManager = async (cwd: string, preset?: PackageManager): Promise<PackageManager> => {
	...
	const response = await prompts({
		...
		initial: detected ? ['bun', 'pnpm', 'npm', 'yarn'].indexOf(detected) : 0
	});
	...
	return response.manager as PackageManager;
};

export default async function addLint(options: AddLintOptions = {}): Promise<void> {
