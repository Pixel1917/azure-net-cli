...
const createMiddlewareManagerTemplate = (): string => `import { createMiddlewareManager } from '@azure-net/kit';

export const { clientMiddleware, serverMiddleware, executeMiddlewares } = createMiddlewareManager([]);
`;

export default async function createCoreMiddleware(): Promise<void> {
