...
const createPresenterFactoryTemplate = (factoryName) => `import { createAsyncHelpers, createPresenterFactory } from '@azure-net/kit/delivery';
...
const buildDefaultPresenterFactoryConfigEntry = (factoryName) => {
    return `defaultPresenterFactory: '${factoryName}'`;
};
const upsertDefaultPresenterFactoryConfig = (content, factoryName) => {
    ...
    if (/defaultPresenterFactory\s*:/.test(nextContent)) {
        ...
        nextContent = nextContent.replace(/export\s+default\s*\{([\s\S]*)\}\s*;?/, (_full, body) => {
...
export default async function createPresenterFactory() {
