...
const parseRepositoryInterfaceMethods = (content) => {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        if (!methodName || paramsChunk === undefined || !responseTypeChunk)
            continue;
...
const parseRepositoryHttpMethods = (content) => {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        if (httpMatch?.[1])
            methods.set(methodName, httpMatch[1]);
    }
    return methods;
};
const getSharedPresenterFactories = async () => {
...
const getSharedSchemaFactories = async () => {
    const { sharedContext } = await getSharedState();
...
const resolveDomainInterfaceFile = async ({ contextName, domainName, interfaceName }) => {
...
export default async function generatePresenter(options = {}) {
    ...
    if (!domainPortsInterfacePath) {
        console.error(`❌ Repository interface "${useCasesInfo.repositoryInterfaceName}" was not found in ${path.join(process.cwd(), 'src', 'app', contextName, 'layers', 'domain', useCasesInfo.domainName, 'ports')}`);
        process.exitCode = 1;
        return;
    }
    const repositoryInterfaceContent = await fs.readFile(domainPortsInterfacePath, 'utf-8');
    const repositoryMethods = parseRepositoryInterfaceMethods(repositoryInterfaceContent).filter((method) => useCasesInfo.methods.includes(method.name));
...
`;
    ...
    const typeImports = normalizeTypeImports(repositoryMethods.flatMap((method) => [method.responseType, ...method.params.map((item) => item.type)]));
